...
## 当前状态
项目已完成阶段 0 骨架、最小阶段 1 闭环，以及阶段 2 的一组基础能力：交互式 CLI、destructive 确认、retrieval metadata/filter、文件清单命令、配置化确认策略。当前环境下向量层继续使用 `MilvusLiteStore` 兼容封装，在 `milvus-lite` 不可安装时自动回退到本地文件向量索引。

## 下一步
1. 评估会话历史持久化和恢复方案
2. 继续推进阶段 2 的图结构清理和职责拆分
